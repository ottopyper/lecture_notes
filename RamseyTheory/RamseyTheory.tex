...
We seek a $\U$ with $\U + \U = \U$, which we call an \undf{idempotent}. Of course, such a $\U$ cannot be principal, because $\tilde{n} + \tilde{n} = \widetilde{2n}\ne \tilde{n}$. It turns out that this is exactly what we need, and moreover that we now have exactly enough to find one.
...
A $2$-colouring of $\N^\bom$ corresponds to a partition $Y\cup Y^c = \N^\om$, some $Y\subset \N^\bom$. Say $Y \in \N^\bom$ is \undf{Ramsey} if $\exists M \in \N^\bom$ with $M^\bom\subset Y$ or $M^\bom\subset Y^c$. So Prop 1. says that not all sets $Y$ are Ramsey.
...
\begin{remark*}
    ...
    The topology on $\N^\bom$ is the \undf{product topology}, also called the \undf{usual topology}, or the \undf{$\tau$-topology}.
\end{remark*}
...
\underline{Reminder}: In a topological space $X$, the \undf{Borel sets} are the smallest family of subsets of $X$ that include the open sets and are closed under complement and countable union.
...
\begin{remark*}[Sociology]
    ...
    For fixed $Y \subset \N^\bom$, and $M \in \N^\bom$, $A \in \N^\blom$, say $M$ \undf{accepts} $A$ (into $Y$) if $(A,M)^\bom \subset Y$, and say $M$ \undf{rejects} $A$ if no $L \in M^\bom$ accepts $A$.
...
\begin{lemma}[Galvin-Prikry Lemma]
...
\begin{defin*}
    The \undf{$\ast$-topology} or \undf{Ellentuck topology} or \undf{Mathias topology} on $\N^\bom$ has basic open sets $(A,M)^\bom$, each $A \in \N^\blom$, $M \in \N^\bom$. Note that this \it{is} a base for a topology:
    \begin{itemize}
...
We say that $Y\subset \N^\bom$ is \undf{completely Ramsey} if for all $A \in \N^\blom$ there exists $L \in M^\bom$ such that $(A,L)^\bom\subset Y$ or $(A,L)^\bom \subset Y^c$. Note that this is at least as strong as being Ramsey, and in fact it is trivially a strictly stronger notion, as we shall now see.
...
\underline{Recall}: A subset $Y$ of topological space $X$ is called \undf{nowhere dense} if it is not dense in any (non-empty) open set, \it{i.e.} $\bar{Y}$ has empty interior, \it{i.e.} for any (non-empty) open set $O\subset X$, there exists a non-empty open $O'\subset O$ with $O' \cap Y = \emptyset$.
...
Say a subset $Y$ of topological space $X$ is \undf{meagre} if $Y = \bigcup_{n=1}^{\infty} Y_n$, with each $Y_n$ nowhere dense.
...
\begin{defin*}[Baire Set]
    A subset $Y$ of a topological space $X$ is called \undf{Baire}, or \undf{has the property of Baire}, if $Y = O \Delta M$, for some $O$ open and $M$ meagre. We can think of this as ``nearly open''.
\end{defin*}
\begin{remark*}[Examples]\ 
    \begin{enumerate}
    ...
    \end{enumerate}
    In fact, the Baire sets form a \undf{$\sigma$-algebra}, \it{i.e.} they are closed under complements and countable unions. Indeed:
    \begin{itemize}
