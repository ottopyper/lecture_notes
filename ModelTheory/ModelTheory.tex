...
Let $\alpha$ be a limit ordinal. A collection $(\M_i){i<\alpha}$ of $\L$-structures is a \undf{chain} if $\M_i \subset \M_{i+1}$ for all $i < \alpha$, and if $\beta < \alpha$ is a limit then $\M_i \subset \M_\beta$ for all $i < \beta$.
